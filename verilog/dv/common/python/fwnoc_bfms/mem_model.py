...
class MemModel(object):
    ...
    def read_word(self, addr) -> int:
        """Read a data-width value to memory"""
        page = self._getpage(addr)
        if self.data_width == 32:
            pageaddr = (addr & 0xFFFC)
        elif self.data_width == 64:
            pageaddr = (addr & 0xFFF8)
    ...
    def write_word(self, addr, data, mask):
        ...
        print("write_word: addr=%08x data=%08x mask=%02x" % (addr, data, mask))
        for i in range(int(self.data_width/8)):
            if self.little_endian:
                if (mask & 1<<i) != 0:
                ...
                if (mask & 1<<(int(self.data_width/8)-i-1)) != 0:
                    print("set pagedata[%d] = %02x" % (pageaddr,((data >> self.data_width-8) & 0xFF)))
                    page.mem[pageaddr] = ((data >> self.data_width-8) & 0xFF)
                data <<= 8
            pageaddr += 1
